...
namespace vMenuClient {
    public class EventManager : BaseScript {
        ...
        /// </summary>
        async void SetAppearanceOnFirstSpawn()
        {
            if (firstSpawn) {
                firstSpawn = false;
                ...
                if (addons.ContainsKey("vehicles")) {
                    foreach (string addon in addons["vehicles"]) {
                        if (!VehicleSpawner.AddonVehicles.ContainsKey(addon)) {
                            ...
                            Debug.WriteLine(
                                $"[vMenu] [Error] Your addons.json file contains 2 or more entries with the same vehicle name! ({addon}) Please remove duplicate lines!");
                        }
                    }
                }
                ...
                if (addons.ContainsKey("weapons")) {
                    foreach (string addon in addons["weapons"]) {
                        if (!WeaponOptions.AddonWeapons.ContainsKey(addon)) {
                            ...
                            Debug.WriteLine(
                                $"[vMenu] [Error] Your addons.json file contains 2 or more entries with the same weapon name! ({addon}) Please remove duplicate lines!");
                        }
                    }
                }
                ...
                if (addons.ContainsKey("peds")) {
                    foreach (string addon in addons["peds"]) {
                        if (!PlayerAppearance.AddonPeds.ContainsKey(addon)) {
                            ...
                            Debug.WriteLine(
                                $"[vMenu] [Error] Your addons.json file contains 2 or more entries with the same ped name! ({addon}) Please remove duplicate lines!");
                        }
                    }
                }
            }
        ...
        void UpdateBanList(string list)
        {
        ...
        /// </summary>
        async void UpdateWeatherParticles()
        {
        ...
        void SetClouds(float opacity, string cloudsType)
        {
        ...
        async void SummonPlayer(string targetPlayer)
        {
        ...
        /// <summary>
        ///     Clear the area around the provided x, y, z coordinates. Clears everything like (destroyed) objects, peds, (ai)
        ///     vehicles, etc.
        ...
        async void GetOutOfCar(int vehNetId, int vehicleOwnedBy)
        {
                ...
                if (DoesEntityExist(veh)) {
                    ...
                    if (Game.PlayerPed.IsInVehicle(vehicle) && vehicleOwnedBy != Game.Player.ServerId) {
                        if (!vehicle.IsStopped) {
                            Notify.Alert(
                                "The owner of this vehicle is reclaiming their personal vehicle. You will be kicked from this vehicle in about 10 seconds. Stop the vehicle now to avoid taking damage.",
                                false);
                        }
        ...
        /// <summary>
        ///     Updates the teleports locations data from the server side locations.json, because that doesn't update client side
        ///     on change.
        /// </summary>
        /// <param name="jsonData"></param>
        void UpdateTeleportLocations(string jsonData)
        {
