...
namespace vMenuClient {
    ...
    class FunctionsController : BaseScript {
        const float voiceIndicatorWidth = 0.02f;
        const float voiceIndicatorHeight = 0.041f;
        const float voiceIndicatorMutedWidth = voiceIndicatorWidth + 0.0021f;
        public const string clothingAnimationDecor = "clothing_animation_type";

        const string snowball_anim_dict = "anim@mp_snowball";
        const string snowball_anim_name = "pickup_snowball";
        readonly uint snowball_hash = (uint) GetHashKey("weapon_snowball");
        float cameraRotationHeading;
        bool clothingAnimationReverse;
        float clothingOpacity = 1f;
        uint crossing = 1;
        string crossingName = "";

        // show location variables
        Vector3 currentPos = Game.PlayerPed.Position;
        List<int> deadPlayers = new List<int>();
        float heading;
        int LastVehicle;
        Vector3 nodePos = Game.PlayerPed.Position;
        List<Ped> peds = new List<Ped>();
        List<Prop> props = new List<Prop>();
        float safeZoneSizeX = 1 / GetSafeZoneSize() / 3.0f - 0.358f;
        bool showSnowballInfo;
        bool stopPedsLoop;
        ...
        bool stopVehiclesLoop;
        string suffix = "";
        bool SwitchedVehicle;
        List<Vehicle> vehicles = new List<Vehicle>();
        int voiceCycle = 1;
        int voiceTimer;
        List<int> waypointPlayerIdsToRemove = new List<int>();
        ...
        async Task PlayerOptions()
        {
            ...
            if (MainMenu.PermissionsSetupComplete && MainMenu.PlayerOptionsMenu != null && IsAllowed(Permission.POMenu)) {
                ...
                if (MainMenu.MpPedCustomizationMenu != null && MainMenu.MpPedCustomizationMenu.appearanceMenu != null &&
                    MainMenu.MpPedCustomizationMenu.faceShapeMenu != null && MainMenu.MpPedCustomizationMenu.createCharacterMenu != null &&
                    MainMenu.MpPedCustomizationMenu.inheritanceMenu != null && MainMenu.MpPedCustomizationMenu.propsMenu != null &&
                    MainMenu.MpPedCustomizationMenu.clothesMenu != null && MainMenu.MpPedCustomizationMenu.tattoosMenu != null) {
    ...
    #region shared player options and vehicle options
        ...
        /// </summary>
        /// <returns></returns>
        async Task DoPlayerAndVehicleChecks()
        {
    ...
    #region Weather Options
        async Task WeatherOptions()
        {
            await Delay(1000);
            if (MainMenu.PermissionsSetupComplete && MainMenu.WeatherOptionsMenu != null && IsAllowed(Permission.WOMenu) &&
                GetSettingsBool(Setting.vmenu_enable_weather_sync)) {
                ...
                if (MainMenu.VoiceChatSettingsMenu.EnableVoicechat && IsAllowed(Permission.VCEnable)) {
                    ...
                    if (MainMenu.VoiceChatSettingsMenu.ShowVoiceStatus) {
                        ...
                        if (NetworkIsPlayerTalking(Game.Player.Handle)) {
                            DrawSprite("mpleaderboard", $"leaderboard_audio_{voiceCycle}", 0.008f, 0.985f, voiceIndicatorWidth, voiceIndicatorHeight, 0f, 255,
                                       55, 0, 255);
                        }
                        else {
                            DrawSprite("mpleaderboard", "leaderboard_audio_mute", 0.008f, 0.985f, voiceIndicatorMutedWidth, voiceIndicatorHeight, 0f, 255, 55,
                                       0, 255);
                        }
                    }
                    else {
    ...
    #region Update Time Options Menu (current time display)
        ...
        /// </summary>
        /// <returns></returns>
        async Task TimeOptions()
        {
            if (MainMenu.PermissionsSetupComplete && MainMenu.TimeOptionsMenu != null && IsAllowed(Permission.TOMenu) &&
                GetSettingsBool(Setting.vmenu_enable_time_sync)) {
    ...
    #region Restore player skin & weapons after respawning.
        ...
        /// </summary>
        /// <returns></returns>
        async Task RestorePlayerAfterBeingDead()
        {
            if (MainMenu.PermissionsSetupComplete && Game.PlayerPed.IsDead) {
                if (MainMenu.MiscSettingsMenu != null) {
                    ...
                    if (MainMenu.MiscSettingsMenu.RestorePlayerWeapons && IsAllowed(Permission.MSRestoreWeapons) || MainMenu.WeaponLoadoutsMenu != null &&
                        MainMenu.WeaponLoadoutsMenu.WeaponLoadoutsSetLoadoutOnRespawn && IsAllowed(Permission.WLEquipOnRespawn)) {
                    ...
                    if (MainMenu.MiscSettingsMenu != null && MainMenu.MiscSettingsMenu.RestorePlayerWeapons && IsAllowed(Permission.MSRestoreWeapons) ||
                        MainMenu.WeaponLoadoutsMenu != null && MainMenu.WeaponLoadoutsMenu.WeaponLoadoutsSetLoadoutOnRespawn &&
    ...
    #region Player clothing animations controller.
        async Task PlayerClothingAnimationsController()
        {
            if (MainMenu.PermissionsSetupComplete) {
                    ...
                    if (MainMenu.MiscSettingsMenu != null) {
                        ...
                        foreach (IPlayer p in MainMenu.PlayersList) {
                            ...
                            if (p != null && NetworkIsPlayerActive(p.Handle) && p.Character != null && p.Character.Exists()) {
                                ...
                                {
                                    if (!(p.Character.AttachedBlip == null || !p.Character.AttachedBlip.Exists()) && MainMenu.OnlinePlayersMenu != null &&
    ...
    #region Tick related to animations and interactions in-game
        ...
        /// </summary>
        /// <returns></returns>
        async Task AnimationsAndInteractions()
        {
            if (MainMenu.PermissionsSetupComplete) {
                if (!(MenuController.IsAnyMenuOpen() || MainMenu.DontOpenMenus || !Fading.IsFadedIn || Game.IsPaused || IsPlayerSwitchInProgress() ||
                        ...
                        if (World.NextWeather == Weather.Christmas) {
                            if (!(Game.PlayerPed.IsInVehicle() || Game.PlayerPed.IsDead || !Fading.IsFadedIn || IsPlayerSwitchInProgress() || Game.IsPaused
                               || GetInteriorFromEntity(Game.PlayerPed.Handle) != 0 || !Game.PlayerPed.IsOnFoot || Game.PlayerPed.IsInParachuteFreeFall ||
                                  ...
                                  Game.PlayerPed.IsSprinting || Game.PlayerPed.IsSwimming || Game.PlayerPed.IsSwimmingUnderWater ||
                    ...
                    if (Game.IsControlPressed(0, Control.SwitchVisor)) {
                        int timer = GetGameTimer();
                        while (!(MenuController.IsAnyMenuOpen() || MainMenu.DontOpenMenus || !Fading.IsFadedIn || Game.IsPaused || IsPlayerSwitchInProgress() ||
                                 Game.PlayerPed.IsDead) && Game.IsControlPressed(0, Control.SwitchVisor)) {
                            await Delay(0);
                            Vehicle veh = GetVehicle();
    ...
    #region help message controller
        ...
        /// </summary>
        /// <returns></returns>
        async Task HelpMessageController()
        {
            if (MainMenu.PermissionsSetupComplete) {
    ...
    #region draw model dimensions
        ...
        /// </summary>
        /// <returns></returns>
        async Task ModelDrawDimensions()
        {
    ...
    #region animal ped camera change blocker
        ...
        /// </summary>
        /// <returns></returns>
        async Task AnimalPedCameraChangeBlocker()
        {
    ...
    #region Vehicle Options Tasks
        ...
        async Task VehicleOptions()
        {
            ...
            if (MainMenu.PermissionsSetupComplete && MainMenu.VehicleOptionsMenu != null && IsAllowed(Permission.VOMenu)) {
                    ...
                    if (veh != null && veh.Exists()) {
                        ...
                        if (SwitchedVehicle) {
                            ...
                            if (IsAllowed(Permission.VOChangePlate) &&
                                MainMenu.VehicleOptionsMenu.GetMenu().GetMenuItems()
                                        .Find(mi => mi is MenuListItem li && li.ListItems.Any(liText => liText == GetLabelText("CMOD_PLA_0"))) is MenuListItem
                                    listItem) {
                ...
                if (MainMenu.VehicleOptionsMenu.VehicleEngineAlwaysOn && GetVehicle(true) != null && GetVehicle(true).Exists() &&
                    !Game.PlayerPed.IsInVehicle() && IsAllowed(Permission.VOEngineAlwaysOn)) {
                    await Delay(100);
        ...
        /// </summary>
        /// <returns></returns>
        async Task MoreVehicleOptions()
        {
            if (MainMenu.PermissionsSetupComplete && MainMenu.VehicleOptionsMenu != null && IsPedInAnyVehicle(Game.PlayerPed.Handle, true) &&
                MainMenu.VehicleOptionsMenu.FlashHighbeamsOnHonk && IsAllowed(Permission.VOFlashHighbeamsOnHonk)) {
                Vehicle veh = GetVehicle();
                if (veh != null && veh.Exists()) {
        ...
        async Task VehicleOptionsEveryFrame()
        {
            Vehicle veh = GetVehicle();
            ...
            if (MainMenu.PermissionsSetupComplete && MainMenu.VehicleOptionsMenu != null && MainMenu.VehicleOptionsMenu.VehicleShowHealth && veh != null &&
                veh.Exists()) {
    ...
    #region Misc Settings Menu Tasks
        async void DrawMiscSettingsText()
        {
            if (MainMenu.PermissionsSetupComplete && MainMenu.MiscSettingsMenu != null) {
                // draw coordinates
                if (MainMenu.MiscSettingsMenu.ShowCoordinates && IsAllowed(Permission.MSShowCoordinates)) {
                    ...
                    DrawTextOnScreen($"~r~X~s~ \t\t{x}\n~r~Y~s~ \t\t{y}\n~r~Z~s~ \t\t{z}\n~r~Heading~s~ \t{heading}", 0.5f - 30f / Resolution.Width, 0f, 0.5f,
                                     Alignment.Left, 6, false);
                    ResetScriptGfxAlign();
                }
                ...
                if (MainMenu.MiscSettingsMenu.DrawTimeOnScreen) {
                    ...
                    string timestring = $"{(hour < 10 ? "0" + hour : hour.ToString())}:{(minute < 10 ? "0" + minute : minute.ToString())}";
                    SetScriptGfxAlign(0, 84);
                    SetScriptGfxAlignParams(0f, 0f, 0f, 0f);
    ...
    #region ShowLocation
        ...
        void ShowLocation()
        {
            ...
            DrawTextOnScreen(prefix + World.GetStreetName(currentPos) + suffix, 0.234f + safeZoneSizeX,
                             GetSafeZoneSize() - GetTextScaleHeight(0.48f, 6) - GetTextScaleHeight(0.48f, 6) /*0.925f - safeZoneSizeY*/, 0.48f);

            // Draw the zone name.
            SetTextWrap(0f, 1f);
            DrawTextOnScreen(World.GetZoneLocalizedName(currentPos), 0.234f + safeZoneSizeX,
                             GetSafeZoneSize() - GetTextScaleHeight(0.45f, 6) - GetTextScaleHeight(0.95f, 6) /*0.9485f - safeZoneSizeY*/, 0.45f);

            // Draw the left border for the heading character.
            SetTextWrap(0f, 1f);
            DrawTextOnScreen("~t~|", 0.188f + safeZoneSizeX,
                             GetSafeZoneSize() - GetTextScaleHeight(1.2f, 6) - GetTextScaleHeight(0.4f, 6) /*0.915f - safeZoneSizeY*/, 1.2f, Alignment.Left);

            // Draw the heading character.
            SetTextWrap(0f, 1f);
            DrawTextOnScreen(headingCharacter, 0.208f + safeZoneSizeX,
                             GetSafeZoneSize() - GetTextScaleHeight(1.2f, 6) - GetTextScaleHeight(0.4f, 6) /*0.915f - safeZoneSizeY*/, 1.2f, Alignment.Center);

            // Draw the right border for the heading character.
            SetTextWrap(0f, 1f);
            DrawTextOnScreen("~t~|", 0.228f + safeZoneSizeX,
                             GetSafeZoneSize() - GetTextScaleHeight(1.2f, 6) - GetTextScaleHeight(0.4f, 6) /*0.915f - safeZoneSizeY*/, 1.2f, Alignment.Right);
        }
    #endregion
        ...
        async Task MiscSettings()
        {
            ...
            #region camera angle locking
                    ...
                    if (IsAllowed(Permission.MSDriftMode)) {
                        if (Game.PlayerPed.IsInVehicle()) {
                            ...
                            if (veh != null && veh.Exists() && !veh.IsDead) {
                                ...
                                else if (Game.IsControlJustReleased(0, Control.Sprint) && Game.CurrentInputMode == InputMode.MouseAndKeyboard ||
                ...
                if (GetProfileSetting(221) == 1) // 221 = settings > display > expanded radar
                {
                    ...
                    if (Game.IsControlJustReleased(0, Control.MultiplayerInfo) && Game.IsControlEnabled(0, Control.MultiplayerInfo) &&
                        MainMenu.MiscSettingsMenu.KbRadarKeys && !MenuController.IsAnyMenuOpen() && !IsPauseMenuActive()) {
                ...
                if (MainMenu.MiscSettingsMenu.KbRecordKeys) {
                    if (!IsPauseMenuActive() && IsScreenFadedIn() && !IsPlayerSwitchInProgress() && !MenuController.IsAnyMenuOpen()) {
                        if (Game.CurrentInputMode == InputMode.MouseAndKeyboard) {
                            Control recordKey = MainMenu.MenuToggleKey == Control.ReplayStartStopRecording
                            ...
                            if (!IsRecording()) {
                                if (Game.IsControlJustReleased(0, recordKey)) {
                                    ...
                                    if (recordKey == Control.ReplayStartStopRecording) {
                                        HelpMessage.Custom(
                                            "Press ~INPUT_REPLAY_START_STOP_RECORDING~ to save the recording, press ~INPUT_REPLAY_CLIP_DELETE~ to discard the recording.");
                                    }
                                    else {
                                        HelpMessage.Custom(
                                            "Press ~INPUT_SAVE_REPLAY_CLIP~ to save the recording, press ~INPUT_REPLAY_CLIP_DELETE~ to discard the recording.");
                                    }
                                }
                            }
                            else {
                                    ...
                                    if (longEnough) {
                                        ...
                                        if (IsRecording()) {
                                            ...
                                            if (Game.IsControlJustReleased(0, Control.ReplayStartStopRecording)) {
                                                StartRecording(1);
                                                HelpMessage.Custom(
                                                    "Hold down ~INPUT_MULTIPLAYER_INFO~ and press ~INPUT_REPLAY_START_STOP_RECORDING~ to save the recording, press ~INPUT_SAVE_REPLAY_CLIP~ to discard the recording.");
                                                break;
                                            }
                                        }
                                    }
    ...
    #region Join / Quit notifications (via events)
        ...
        void OnJoinQuitNotification(string playerName, string dropReason)
        {
            if (MainMenu.PermissionsSetupComplete && MainMenu.MiscSettingsMenu != null) {
    ...
    #region Player Appearance
        ...
        async Task UpdateCamera(Camera oldCamera, Vector3 pos, Vector3 pointAt)
        {
            ...
            if (MainMenu.PermissionsSetupComplete && MainMenu.MpPedCustomizationMenu != null) {
                if (Game.PlayerPed.IsInVehicle()) {
                    ...
                    if (MainMenu.MpPedCustomizationMenu.editPedBtn != null && !MainMenu.MpPedCustomizationMenu.editPedBtn.Enabled) {
                        ...
                        MainMenu.MpPedCustomizationMenu.editPedBtn.Description =
                            MainMenu.MpPedCustomizationMenu.editPedBtn.Description.Replace(" ~r~You need to get out of your vehicle before you can use this.",
                                                                                           "");
                    }
                ...
                if (IsMpCharEditorOpen()) {
                    ...
                    while (IsMpCharEditorOpen()) {
                        ...
                        if (MenuController.GetCurrentMenu() == MainMenu.MpPedCustomizationMenu.propsMenu && MenuController.GetCurrentMenu().CurrentIndex == 3 &&
                            !reverseCamera) {
                        ...
                        if ((Game.IsControlPressed(0, Control.ParachuteBrakeLeft) || Game.IsControlPressed(0, Control.ParachuteBrakeRight)) &&
                            !(Game.IsControlPressed(0, Control.ParachuteBrakeLeft) && Game.IsControlPressed(0, Control.ParachuteBrakeRight))) {
                            switch (index) {
                                case 0:
                                    xOffset = 2.2f;
                                    yOffset = -1f;
                        ...
                        Vector3 pos;
                        if (reverseCamera) {
                            pos = GetOffsetFromEntityInWorldCoords(Game.PlayerPed.Handle, (CameraOffsets[index].Key.X + xOffset) * -1f,
                                                                   (CameraOffsets[index].Key.Y + yOffset) * -1f, CameraOffsets[index].Key.Z);
                        }
                        else {
                            pos = GetOffsetFromEntityInWorldCoords(Game.PlayerPed.Handle, CameraOffsets[index].Key.X + xOffset,
                                                                   CameraOffsets[index].Key.Y + yOffset, CameraOffsets[index].Key.Z);
                        }

                        Vector3 pointAt = GetOffsetFromEntityInWorldCoords(Game.PlayerPed.Handle, CameraOffsets[index].Value.X, CameraOffsets[index].Value.Y,
                                                                           CameraOffsets[index].Value.Z);
        ...
        int GetCameraIndex(Menu menu)
        {
            if (menu != null) {
                ...
                if (menu == MainMenu.MpPedCustomizationMenu.clothesMenu) {
                ...
                if (menu == MainMenu.MpPedCustomizationMenu.propsMenu) {
                    switch (menu.CurrentIndex) {
                        case 0: // hats & helmets
                        case 1: // glasses
                        case 2: // misc props
                ...
                if (menu == MainMenu.MpPedCustomizationMenu.appearanceMenu) {
                    switch (menu.CurrentIndex) {
                        case 0: // hair style
                        case 1: // hair color
                ...
                if (menu == MainMenu.MpPedCustomizationMenu.tattoosMenu) {
                    switch (menu.CurrentIndex) {
                        case 0: // head
                            return 1;
                        case 1: // torso
                ...
                if (menu == MainMenu.MpPedCustomizationMenu.faceShapeMenu) {
    ...
    #region player overhead names
        ...
        float playerNamesDistance = GetSettingsFloat(Setting.vmenu_player_names_distance) > 10f ? GetSettingsFloat(Setting.vmenu_player_names_distance) : 500f;
    ...
    #region Personal Vehicle options
        ...
        async void StartMessageCooldown()
        {
        ...
        /// </summary>
        /// <returns></returns>
        async Task PersonalVehicleOptions()
        {
            if (MainMenu.PermissionsSetupComplete && MainMenu.PersonalVehicleMenu != null && IsAllowed(Permission.PVLockDoors) &&
                    ...
                    if (Game.PlayerPed.Position.DistanceToSquared(MainMenu.PersonalVehicleMenu.CurrentPersonalVehicle.Position) < 650.0f) {
                        if (Game.IsControlJustReleased(0, Control.VehicleHorn)) {
                            ...
                            if (GetGameTimer() - time < 500) {
                                ...
                                await Delay(100);
                                bool lockDoors =
                                    !GetVehicleDoorsLockedForPlayer(MainMenu.PersonalVehicleMenu.CurrentPersonalVehicle.Handle, Game.PlayerPed.Handle);
                        ...
                        if (!messageCooldown) {
                            HelpMessage.Custom("When you are close to your personal vehicle, you can double tap ~INPUT_VEH_HORN~ to lock or unlock it.", 10000,
                                               true);
