...
namespace vMenuClient {
    public class BannedPlayers {
        ...
        void CreateMenu()
        {
            ...
            menu.ButtonPressHandlers.Add(new Menu.ButtonPressHandler(Control.Jump, Menu.ControlPressCheckType.JUST_RELEASED, async (a, b) => {
                if (banlist.Count > 1) {
                    ...
                    if (string.IsNullOrEmpty(filterText)) {
                        ...
                        menu.FilterMenuItems(item => item.ItemData is BanRecord br &&
                                                     (br.playerName.ToLower().Contains(filterText.ToLower()) ||
                                                      br.uuid.ToLower().Contains(filterText.ToLower())));
            ...
            bannedPlayer.AddMenuItem(
                new MenuItem("~r~Unban",
                             "~r~Warning, unbanning the player can NOT be undone. You will NOT be able to ban them again until they re-join the server. Are you absolutely sure you want to unban this player? ~s~Tip: Tempbanned players will automatically get unbanned if they log on to the server after their ban date has expired."));
            ...
            bannedPlayer.OnIndexChange += (sender, oldItem, newItem, oldIndex, newIndex) => { bannedPlayer.GetMenuItems()[5].Label = ""; };
                ...
                if (index == 5 && IsAllowed(Permission.OPUnban)) {
                    if (item.Label == "Are you sure?") {
                        if (banlist.Contains(currentRecord)) {
                            ...
                            Notify.Error(
                                "Somehow you managed to click the unban button but this ban record you're apparently viewing does not even exist. Weird...");
                        }
                    }
                    else {
            ...
            menu.OnItemSelect += (sender, item, index) => {
                ...
                MenuItem nameItem = bannedPlayer.GetMenuItems()[0];
                MenuItem bannedByItem = bannedPlayer.GetMenuItems()[1];
                MenuItem bannedUntilItem = bannedPlayer.GetMenuItems()[2];
                MenuItem playerIdentifiersItem = bannedPlayer.GetMenuItems()[3];
                MenuItem banReasonItem = bannedPlayer.GetMenuItems()[4];
                ...
                bannedUntilItem.Description = "This player is banned until: " + currentRecord.bannedUntil.Date;
                ...
                MenuItem unbanPlayerBtn = bannedPlayer.GetMenuItems()[5];
        ...
        public void UpdateBans()
        {
            ...
            foreach (BanRecord ban in banlist) {
                MenuItem recordBtn =
                    new MenuItem(ban.playerName,
                                 $"~y~{ban.playerName}~s~ was banned by ~y~{ban.bannedBy}~s~ until ~y~{ban.bannedUntil}~s~ for ~y~{ban.banReason}~s~.") {
                        Label = "→→→",
                        ItemData = ban
                    };
        ...
        ///     We'll just assume that worked fine, so remove the item from our local list, we'll re-sync once the menu is
        ///     re-opened.
        /// </summary>
        /// <param name="index"></param>
        void UnbanPlayer(int index)
        {
        ...
        public static BanRecord JsonToBanRecord(dynamic banRecordJsonObject)
        {
            ...
            foreach (JProperty brValue in banRecordJsonObject) {
                string key = brValue.Name;
                JToken value = brValue.Value;
                if (key == "playerName") {
