...
namespace vMenuClient {
    public class VehicleSpawner {
        ...
        #region initial setup.
            ...
            MenuCheckboxItem spawnInVeh =
                new MenuCheckboxItem("Spawn Inside Vehicle", "This will teleport you into the vehicle when you spawn it.", SpawnInVehicle);
            MenuCheckboxItem replacePrev = new MenuCheckboxItem("Replace Previous Vehicle",
                ...
                if (AddonVehicles != null) {
                    if (AddonVehicles.Count > 0) {
                        ...
                        for (int cat = 0; cat < 23; cat++) {
                            ...
                            MenuItem categoryBtn = new MenuItem(GetLabelText($"VEH_CLASS_{cat}"),
                                                                $"Spawn an addon vehicle from the {GetLabelText($"VEH_CLASS_{cat}")} class.") {Label = "→→→"};
                            ...
                            foreach (KeyValuePair<string, uint> veh in AddonVehicles.Where(v => GetVehicleClassFromName(v.Value) == cat)) {
                                ...
                                if (IsModelInCdimage(veh.Value)) {
                                    ...
                                    carBtn.Description =
                                        "This vehicle is not available. Please ask the server owner to check if the vehicle is being streamed correctly.";
            ...
            #region Add vehicles per class
                ...
                foreach (string veh in VehicleData.Vehicles.VehicleClasses[className]) {
                    ...
                    for (int itemIndex = 0; itemIndex < vehicleClassMenu.Size; itemIndex++) {
                        // If it matches...
                        if (vehicleClassMenu.GetMenuItems()[itemIndex].Text == vehName) {
                            ...
                            if (DoesModelExist(veh)) {
                                MenuItem vehBtn = new MenuItem(vehName) {
                                    Enabled = true,
                ...
                vehicleClassMenu.OnMenuOpen += m => { HandleStatsPanel(m, m.GetCurrentMenuItem()); };

                vehicleClassMenu.OnIndexChange += (m, oldItem, newItem, oldIndex, newIndex) => { HandleStatsPanel(m, newItem); };
            }
        #endregion
