...
namespace vMenuClient
{
    public class PlayerOptions
    {
        ...
        private Menu CustomDrivingStyleMenu = new Menu(LM.Get("Driving Style"), LM.Get("Custom Driving Style"));
        ...
        private void CreateMenu()
        {
            ...
            // Create the menu.
            menu = new Menu(Game.Player.Name, LM.Get("Player Options"));

            // Create all checkboxes.
            MenuCheckboxItem playerGodModeCheckbox = new MenuCheckboxItem(LM.Get("Godmode"), LM.Get("Makes you invincible."), PlayerGodMode);
            MenuCheckboxItem invisibleCheckbox = new MenuCheckboxItem(LM.Get("Invisible"), LM.Get("Makes you invisible to yourself and others."), PlayerInvisible);
            MenuCheckboxItem unlimitedStaminaCheckbox = new MenuCheckboxItem(LM.Get("Unlimited Stamina"), LM.Get("Allows you to run forever without slowing down or taking damage."), PlayerStamina);
            MenuCheckboxItem fastRunCheckbox = new MenuCheckboxItem(LM.Get("Fast Run"), LM.Get("Get ~g~Snail~s~ powers and run very fast!"), PlayerFastRun);
            SetRunSprintMultiplierForPlayer(Game.Player.Handle, (PlayerFastRun && IsAllowed(Permission.POFastRun) ? 1.49f : 1f));
            MenuCheckboxItem fastSwimCheckbox = new MenuCheckboxItem(LM.Get("Fast Swim"), LM.Get("Get ~g~Snail 2.0~s~ powers and swim super fast!"), PlayerFastSwim);
            SetSwimMultiplierForPlayer(Game.Player.Handle, (PlayerFastSwim && IsAllowed(Permission.POFastSwim) ? 1.49f : 1f));
            MenuCheckboxItem superJumpCheckbox = new MenuCheckboxItem(LM.Get("Super Jump"), LM.Get("Get ~g~Snail 3.0~s~ powers and jump like a champ!"), PlayerSuperJump);
            MenuCheckboxItem noRagdollCheckbox = new MenuCheckboxItem(LM.Get("No Ragdoll"), LM.Get("Disables player ragdoll, makes you not fall off your bike anymore."), PlayerNoRagdoll);
            MenuCheckboxItem neverWantedCheckbox = new MenuCheckboxItem(LM.Get("Never Wanted"), LM.Get("Disables all wanted levels."), PlayerNeverWanted);
            MenuCheckboxItem everyoneIgnoresPlayerCheckbox = new MenuCheckboxItem(LM.Get("Everyone Ignore Player"), LM.Get("Everyone will leave you alone."), PlayerIsIgnored);
            MenuCheckboxItem playerStayInVehicleCheckbox = new MenuCheckboxItem(LM.Get("Stay In Vehicle"), LM.Get("When this is enabled, NPCs will not be able to drag you out of your vehicle if they get angry at you."), PlayerStayInVehicle);
            MenuCheckboxItem playerFrozenCheckbox = new MenuCheckboxItem(LM.Get("Freeze Player"), LM.Get("Freezes your current location."), PlayerFrozen);

            // Wanted level options
            List<string> wantedLevelList = new List<string> { LM.Get("No Wanted Level"), "1", "2", "3", "4", "5" };
            MenuListItem setWantedLevel = new MenuListItem(LM.Get("Set Wanted Level"), wantedLevelList, GetPlayerWantedLevel(Game.Player.Handle), LM.Get("Set your wanted level by selecting a value, and pressing enter."));
            MenuListItem setArmorItem = new MenuListItem(LM.Get("Set Armor Type"), new List<string> { LM.Get("No Armor"), GetLabelText("WT_BA_0"), GetLabelText("WT_BA_1"), GetLabelText("WT_BA_2"), GetLabelText("WT_BA_3"), GetLabelText("WT_BA_4"), }, 0, "Set the armor level/type for your player.");

            MenuItem healPlayerBtn = new MenuItem(LM.Get("Heal Player"), LM.Get("Give the player max health."));
            MenuItem cleanPlayerBtn = new MenuItem(LM.Get("Clean Player Clothes"), LM.Get("Clean your player clothes."));
            MenuItem dryPlayerBtn = new MenuItem(LM.Get("Dry Player Clothes"), LM.Get("Make your player clothes dry."));
            MenuItem wetPlayerBtn = new MenuItem(LM.Get("Wet Player Clothes"), LM.Get("Make your player clothes wet."));
            MenuItem suicidePlayerBtn = new MenuItem(LM.Get("~r~Commit Suicide"), LM.Get("Kill yourself by taking the pill. Or by using a pistol if you have one."));

            Menu vehicleAutoPilot = new Menu(LM.Get("Auto Pilot"), LM.Get("Vehicle auto pilot options."));

            MenuController.AddSubmenu(menu, vehicleAutoPilot);

            MenuItem vehicleAutoPilotBtn = new MenuItem(LM.Get("Vehicle Auto Pilot Menu"), LM.Get("Manage vehicle auto pilot options."))
            {
                Label = "→→→"
            };

            List<string> drivingStyles = new List<string>() { LM.Get("Normal"), LM.Get("Rushed"), LM.Get("Avoid highways"), LM.Get("Drive in reverse"), LM.Get("Custom") };
            MenuListItem drivingStyle = new MenuListItem(LM.Get("Driving Style"), drivingStyles, 0, LM.Get("Set the driving style that is used for the Drive to Waypoint and Drive Around Randomly functions."));

            // Scenarios (list can be found in the PedScenarios class)
            MenuListItem playerScenarios = new MenuListItem(LM.Get("Player Scenarios"), PedScenarios.Scenarios, 0, LM.Get("Select a scenario and hit enter to start it. Selecting another scenario will override the current scenario. If you're already playing the selected scenario, selecting it again will stop the scenario."));
            MenuItem stopScenario = new MenuItem(LM.Get("Force Stop Scenario"), LM.Get("This will force a playing scenario to stop immediately, without waiting for it to finish it's 'stopping' animation."));
            #endregion
            ...
            if (IsAllowed(Permission.POVehicleAutoPilotMenu))
            {
                ...
                MenuItem startDrivingWaypoint = new MenuItem(LM.Get("Drive To Waypoint"), LM.Get("Make your player ped drive your vehicle to your waypoint."));
                MenuItem startDrivingRandomly = new MenuItem(LM.Get("Drive Around Randomly"), LM.Get("Make your player ped drive your vehicle randomly around the map."));
                MenuItem stopDriving = new MenuItem(LM.Get("Stop Driving"), LM.Get("The player ped will find a suitable place to stop the vehicle. The task will be stopped once the vehicle has reached the suitable stop location."));
                MenuItem forceStopDriving = new MenuItem(LM.Get("Force Stop Driving"), LM.Get("This will stop the driving task immediately without finding a suitable place to stop."));
                MenuItem customDrivingStyle = new MenuItem(LM.Get("Custom Driving Style"), LM.Get("Select a custom driving style. Make sure to also enable it by selecting the 'Custom' driving style in the driving styles list.")) { Label = "→→→" };
                ...
                {
                    { 0, LM.Get("Stop before vehicles") },
                    { 1, LM.Get("Stop before peds") },
                    { 2, LM.Get("Avoid vehicles") },
                    { 3, LM.Get("Avoid empty vehicles") },
                    { 4, LM.Get("Avoid peds") },
                    { 5, LM.Get("Avoid objects") },

                    { 7, LM.Get("Stop at traffic lights") },
                    { 8, LM.Get("Use blinkers") },
                    { 9, LM.Get("Allow going wrong way") },
                    { 10, LM.Get("Go in reverse gear") },

                    { 18, LM.Get("Use shortest path") },

                    { 22, LM.Get("Ignore roads") },

                    { 24, LM.Get("Ignore all pathing") },

                    { 29, LM.Get("Avoid highways (if possible)") },
                };
                for (var i = 0; i < 31; i++)
                {
                    string name = LM.Get("~r~Unknown Flag");
                    if (knownNames.ContainsKey(i))
                    {
                        name = knownNames[i];
                    }
                    MenuCheckboxItem checkbox = new MenuCheckboxItem(name, LM.Get("Toggle this driving style flag."), false);
                ...
                CustomDrivingStyleMenu.OnCheckboxChange += (sender, item, index, _checked) =>
                {
                    ...
                    {
                        Notify.Custom(LM.Get("Driving style updated."));
                        SetDriveTaskDrivingStyle(Game.PlayerPed.Handle, style);
                    }
                    else
                    {
                        Notify.Custom(LM.Get("Driving style NOT updated because you haven't enabled the Custom driving style in the previous menu."));
                    }
                };
                ...
                vehicleAutoPilot.OnItemSelect += async (sender, item, index) =>
                {
                    if (Game.PlayerPed.IsInVehicle() && item != stopDriving && item != forceStopDriving)
                    {
                            ...
                            if (Game.PlayerPed.CurrentVehicle.Driver == Game.PlayerPed)
                            {
                                if (item == startDrivingWaypoint)
                                {
                                    if (IsWaypointActive())
                                    {
                                        int style = GetStyleFromIndex(drivingStyle.ListIndex);
                                        DriveToWp(style);
                                        Notify.Info(LM.Get("Your player ped is now driving the vehicle for you. You can cancel any time by pressing the Stop Driving button. The vehicle will stop when it has reached the destination."));
                                    }
                                    else
                                    {
                                        Notify.Error(LM.Get("You need a waypoint before you can drive to it!"));
                                    }
                                ...
                                else if (item == startDrivingRandomly)
                                {
                                    int style = GetStyleFromIndex(drivingStyle.ListIndex);
                                    DriveWander(style);
                                    Notify.Info(LM.Get("Your player ped is now driving the vehicle for you. You can cancel any time by pressing the Stop Driving button."));
                                }
                            }
                            else
                            {
                        ...
                        {
                            Notify.Error(LM.Get("Your vehicle is broken or it does not exist!"));
                        }
                    }
                    else if (item != stopDriving && item != forceStopDriving)
                    {
                        Notify.Error(LM.Get("You need to be in a vehicle first!"));
                    }
                    if (item == stopDriving)
                    {
                        if (Game.PlayerPed.IsInVehicle())
                        {
                            ...
                            if (veh != null && veh.Exists() && !veh.IsDead)
                            {
                                ...
                                {
                                    Notify.Info(LM.Get("The player ped will find a suitable place to park the car and will then stop driving. Please wait."));
                                    ...
                                    ClearPedTasks(Game.PlayerPed.Handle);
                                    Notify.Info(LM.Get("The player ped has stopped driving."));
                                }
                            }
                        }
                        else
                        {
                            ClearPedTasks(Game.PlayerPed.Handle);
                            Notify.Alert(LM.Get("Your ped is not in any vehicle."));
                        }
                    }
                    else if (item == forceStopDriving)
                    {
                        ClearPedTasks(Game.PlayerPed.Handle);
                        Notify.Info(LM.Get("Driving task cancelled."));
                    }
                };
            ...
            menu.OnItemSelect += (sender, item, index) =>
            {
                ...
                else if (item == healPlayerBtn)
                {
                    ...
                    Notify.Success(LM.Get("Player healed."));
                }
                else if (item == cleanPlayerBtn)
                {
                    Game.PlayerPed.ClearBloodDamage();
                    Notify.Success(LM.Get("Player clothes have been cleaned."));
                }
                ...
                else if (item == wetPlayerBtn)
                {
                    ...
                    Notify.Success(LM.Get("Player is now wet."));
                }
