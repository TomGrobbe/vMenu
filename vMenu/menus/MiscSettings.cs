...
namespace vMenuClient {
    public class MiscSettings {
        internal static List<ConfigManager.TeleportLocation> TpLocations = new List<ConfigManager.TeleportLocation>();

        List<Blip> blips = new List<Blip>();

        Menu developerToolsMenu;

        // Variables
        Menu menu;
        Menu teleportOptionsMenu;
        ...
        internal int LastTimeCycleModifierIndex { get; } = UserDefaults.MiscLastTimeCycleModifierIndex;
        internal int LastTimeCycleModifierStrength { get; } = UserDefaults.MiscLastTimeCycleModifierStrength;


        // keybind states
        public bool KbTpToWaypoint { get; private set; } = UserDefaults.KbTpToWaypoint;

        public int KbTpToWaypointKey { get; } = ConfigManager.GetSettingsInt(ConfigManager.Setting.vmenu_teleport_to_wp_keybind_key) != -1
                                                    ? ConfigManager.GetSettingsInt(ConfigManager.Setting.vmenu_teleport_to_wp_keybind_key)
        ...
        void CreateMenu()
        {
            ...
            MenuCheckboxItem kbTpToWaypoint = new MenuCheckboxItem("Teleport To Waypoint",
                                                                   ...
                                                                   KbTpToWaypoint);
            MenuCheckboxItem kbDriftMode =
                new MenuCheckboxItem("Drift Mode", "Makes your vehicle have almost no traction while holding left shift on keyboard, or X on controller.",
                                     KbDriftMode);
            MenuCheckboxItem kbRecordKeys = new MenuCheckboxItem("Recording Controls",
                                                                 "Enables or disables the recording (gameplay recording for the Rockstar editor) hotkeys on both keyboard and controller.",
                                                                 KbRecordKeys);
            MenuCheckboxItem kbRadarKeys = new MenuCheckboxItem("Minimap Controls",
                                                                "Press the Multiplayer Info (z on keyboard, down arrow on controller) key to switch between expanded radar and normal radar.",
                                                                KbRadarKeys);
            MenuCheckboxItem kbPointKeysCheckbox = new MenuCheckboxItem("Finger Point Controls",
            ...
            MenuCheckboxItem rightAlignMenu = new MenuCheckboxItem("Right Align Menu",
                                                                   ...
                                                                   MiscRightAlignMenu);
            MenuCheckboxItem disablePms = new MenuCheckboxItem("Disable Private Messages",
                                                               ...
                                                               MiscDisablePrivateMessages);
            MenuCheckboxItem disableControllerKey = new MenuCheckboxItem("Disable Controller Support",
                                                                         "This disables the controller menu toggle key. This does NOT disable the navigation buttons.",
                                                                         MiscDisableControllerSupport);
            MenuCheckboxItem speedKmh =
                new MenuCheckboxItem("Show Speed KM/H", "Show a speedometer on your screen indicating your speed in KM/h.", ShowSpeedoKmh);
            MenuCheckboxItem speedMph =
                new MenuCheckboxItem("Show Speed MPH", "Show a speedometer on your screen indicating your speed in MPH.", ShowSpeedoMph);
            ...
            MenuCheckboxItem showLocation = new MenuCheckboxItem("Location Display",
            ...
            MenuCheckboxItem joinQuitNotifs = new MenuCheckboxItem("Join / Quit Notifications",
                                                                   "Receive notifications when someone joins or leaves the server.", JoinQuitNotifications);
            MenuCheckboxItem deathNotifs =
                new MenuCheckboxItem("Death Notifications", "Receive notifications when someone dies or gets killed.", DeathNotifications);
            ...
            MenuCheckboxItem vehModelDimensions = new MenuCheckboxItem("Show Vehicle Dimensions",
                                                                       "Draws the model outlines for every vehicle that's currently close to you.",
                                                                       ShowVehicleModelDimensions);
            MenuCheckboxItem propModelDimensions = new MenuCheckboxItem("Show Prop Dimensions",
                                                                        "Draws the model outlines for every prop that's currently close to you.",
                                                                        ShowPropModelDimensions);
            MenuCheckboxItem pedModelDimensions = new MenuCheckboxItem("Show Ped Dimensions",
                                                                       "Draws the model outlines for every ped that's currently close to you.",
                                                                       ShowPedModelDimensions);
            MenuCheckboxItem showEntityHandles = new MenuCheckboxItem("Show Entity Handles",
                                                                      "Draws the the entity handles for all close entities (you must enable the outline functions above for this to work).",
                                                                      ShowEntityHandles);
            MenuCheckboxItem showEntityModels = new MenuCheckboxItem("Show Entity Models",
                                                                     "Draws the the entity models for all close entities (you must enable the outline functions above for this to work).",
                                                                     ShowEntityModels);
            MenuCheckboxItem showEntityNetOwners = new MenuCheckboxItem("Show Network Owners",
                                                                        "Draws the the entity net owner for all close entities (you must enable the outline functions above for this to work).",
                                                                        ShowEntityNetOwners);
            MenuSliderItem dimensionsDistanceSlider =
                new MenuSliderItem("Show Dimensions Radius", "Show entity model/handle/dimension draw range.", 0, 20, 20, false);

            MenuItem clearArea =
                new MenuItem("Clear Area",
            ...
            MenuCheckboxItem lockCamX = new MenuCheckboxItem("Lock Camera Horizontal Rotation",
                                                             "Locks your camera horizontal rotation. Could be useful in helicopters I guess.", false);
            MenuCheckboxItem lockCamY = new MenuCheckboxItem("Lock Camera Vertical Rotation",
                                                             "Locks your camera vertical rotation. Could be useful in helicopters I guess.", false);
            ...
            MenuItem quitSession =
                new MenuItem("Quit Session", "Leaves you connected to the server, but quits the network session. ~r~Can not be used when you are the host.");
            MenuItem rejoinSession = new MenuItem("Re-join Session",
            ...
            MenuCheckboxItem enableTimeCycle = new MenuCheckboxItem("Enable Timecycle Modifier",
                                                                    "Enable or disable the timecycle modifier from the list below.", TimecycleEnabled);
            ...
            MenuListItem timeCycles = new MenuListItem("TM", timeCycleModifiersListData,
                                                       MathUtil.Clamp(LastTimeCycleModifierIndex, 0, Math.Max(0, timeCycleModifiersListData.Count - 1)),
                                                       "Select a timecycle modifier and enable the checkbox above.");
            MenuSliderItem timeCycleIntensity = new MenuSliderItem("Timecycle Modifier Intensity", "Set the timecycle modifier intensity.", 0, 20,
                                                                   LastTimeCycleModifierStrength, true);
            ...
            MenuCheckboxItem respawnDefaultCharacter = new MenuCheckboxItem("Respawn As Default MP Character",
            ...
            MenuCheckboxItem restorePlayerAppearance = new MenuCheckboxItem("Restore Player Appearance",
            ...
            MenuCheckboxItem restorePlayerWeapons = new MenuCheckboxItem("Restore Player Weapons",
            ...
            connectionSubmenu.OnItemSelect += (sender, item, index) => {
                if (item == quitGame) {
                    ...
                    if (NetworkIsSessionActive()) {
                        if (NetworkIsHost()) {
                            Notify.Error(
                                "Sorry, you cannot leave the session when you are the host. This would prevent other players from joining/staying on the server.");
                        }
                        else {
                            QuitSession();
                        }
            ...
            if (IsAllowed(Permission.MSTeleportToWp) || IsAllowed(Permission.MSTeleportLocations) || IsAllowed(Permission.MSTeleportToCoord)) {
                ...
                MenuItem saveLocationBtn = new MenuItem("Save Teleport Location",
                ...
                if (IsAllowed(Permission.MSTeleportLocations)) {
                    ...
                    teleportMenu.OnMenuOpen += sender => {
                        if (teleportMenu.Size != TpLocations.Count()) {
                            ...
                            foreach (ConfigManager.TeleportLocation location in TpLocations) {
                                ...
                                MenuItem tpBtn =
                                    new MenuItem(location.name,
                                                 $"Teleport to ~y~{location.name}~n~~s~x: ~y~{x}~n~~s~y: ~y~{y}~n~~s~z: ~y~{z}~n~~s~heading: ~y~{heading}")
                                        {ItemData = location};
                    ...
                    teleportMenu.OnItemSelect += async (sender, item, index) => {
                        if (item.ItemData is ConfigManager.TeleportLocation tl) {
        ...
        #region dev tools menu
            ...
            // model outlines
            if (!ConfigManager.GetSettingsBool(ConfigManager.Setting.vmenu_disable_entity_outlines_tool)) {
            ...
            developerToolsMenu.OnListIndexChange += (sender, item, oldIndex, newIndex, itemIndex) => {
                if (item == timeCycles) {
                    ...
                    if (TimecycleEnabled) {
                        SetTimecycleModifier(TimeCycles.Timecycles[timeCycles.ListIndex]);
                        float intensity = timeCycleIntensity.Position / 20f;
                        SetTimecycleModifierStrength(intensity);
                    }
        ...
        void ToggleBlips(bool enable)
        {
            if (enable) {
                try {
                    foreach (ConfigManager.LocationBlip bl in ConfigManager.GetLocationBlipsData()) {
