...
namespace vMenuClient.menus
{
    public class WeaponOptions
    {
        ...
        private void CreateMenu()
        {
            ...
            var getAllWeapons = new MenuItem("获取全部武器", "获取所有武器。");
            var removeAllWeapons = new MenuItem("移除全部武器", "移除库存中的所有武器。");
            var unlimitedAmmo = new MenuCheckboxItem("无限弹药", "提供无限弹药。", UnlimitedAmmo);
            var noReload = new MenuCheckboxItem("无需换弹", "无需换弹。", NoReload);
            var setAmmo = new MenuItem("设置全部弹药数量", "设置所有武器的弹药数量。");
            var refillMaxAmmo = new MenuItem("补满全部弹药", "将所有武器的弹药补满。");
            var spawnByName = new MenuItem("按名称生成武器", "输入要生成的武器模型名称。");
            ...
            foreach (var addonWeapon in ValidAddonWeapons.AddonWeaponsList)
            {
                ...
                {
                    var addonWeaponMenu = new Menu("武器功能选项", addonWeapon.Name)
                    ...
                    var addonWeaponItem = new MenuItem(addonWeapon.Name, $"打开 ~y~{addonWeapon.Name}~s~ 的选项。")
                    {
                        Label = "→→→",
                    ...
                    if (!vMenuShared.SupplementaryPermissionManager.IsAllowed("WW" + spawnName.ToLower().Replace("weapon_", "")))
                    {
                        ...
                        addonWeaponItem.Description = "服务器所有者已限制访问此项。";
                    }
