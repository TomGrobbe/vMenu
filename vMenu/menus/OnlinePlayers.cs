...
namespace vMenuClient {
    public class OnlinePlayers {
        ...
        void CreateMenu()
        {
            ...
            MenuItem ban = new MenuItem("~r~Ban Player Permanently",
            ...
            MenuItem tempban = new MenuItem("~r~Ban Player Temporarily",
            ...
            playerMenu.OnIndexChange += (sender, oldItem, newItem, oldIndex, newIndex) => { ban.Label = ""; };
            ...
            playerMenu.OnItemSelect += async (sender, item, index) => {
                ...
                if (item == sendMessage) {
                    if (MainMenu.MiscSettingsMenu != null && !MainMenu.MiscSettingsMenu.MiscDisablePrivateMessages) {
                        ...
                        Notify.Error(
                            "You can't send a private message if you have private messages disabled yourself. Enable them in the Misc Settings menu and try again.");
                    }
                }
                // teleport (in vehicle) button
                    ...
                    if (!currentPlayer.IsLocal) {
                        _ = TeleportToPlayer(
                            currentPlayer, item == teleportVeh); // teleport to the player. optionally in the player's vehicle if that button was pressed.
                    }
                    else {
                ...
                // manage the gps route being clicked.
                    ...
                    if (!selectedPedRouteAlreadyActive) {
                        if (currentPlayer.Handle != Game.Player.Handle) {
                            ...
                            Notify.Custom(
                                $"~g~GPS route to ~s~<C>{GetSafePlayerName(currentPlayer.Name)}</C>~g~ is now active, press the ~s~Toggle GPS Route~g~ button again to disable the route.");
                        }
                        else {
        ...
        public async Task UpdatePlayerlist()
        {
            void UpdateStuff()
            {
                ...
                foreach (IPlayer p in MainMenu.PlayersList.OrderBy(a => a.Name)) {
                    MenuItem pItem = new MenuItem($"{GetSafePlayerName(p.Name)}",
                                                  $"Click to view the options for this player. Server ID: {p.ServerId}. Local ID: {p.Handle}.") {
