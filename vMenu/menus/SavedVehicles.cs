...
namespace vMenuClient {
    public class SavedVehicles {
        KeyValuePair<string, VehicleInfo> currentlySelectedVehicle;
        ...
        Menu selectedVehicleMenu = new Menu("Manage Vehicle", "Manage this saved vehicle.");
        ...
        Menu unavailableVehiclesMenu = new Menu("Missing Vehicles", "Unavailable Saved Vehicles");
        ...
        #region Create menus and submenus
            ...
            for (int i = 0; i < 23; i++) {
                ...
                MenuItem categoryButton =
                    new MenuItem(GetLabelText($"VEH_CLASS_{i}"), $"All saved vehicles from the {GetLabelText($"VEH_CLASS_{i}")} category.");
                ...
                categoryMenu.OnMenuClose += sender => { UpdateMenuAvailableCategories(); };

                categoryMenu.OnItemSelect += (sender, item, index) => { UpdateSelectedVehicleMenu(item, sender); };
            }

            MenuItem unavailableModels = new MenuItem("Unavailable Saved Vehicles",
            ...
            MenuItem replaceVehicle = new MenuItem("~r~Replace Vehicle",
            ...
            selectedVehicleMenu.OnItemSelect += async (sender, item, index) => {
                if (item == spawnVehicle) {
                    if (MainMenu.VehicleSpawnerMenu != null) {
                        SpawnVehicle(currentlySelectedVehicle.Value.model, MainMenu.VehicleSpawnerMenu.SpawnInVehicle,
                                     MainMenu.VehicleSpawnerMenu.ReplaceVehicle, false, currentlySelectedVehicle.Value,
                                     currentlySelectedVehicle.Key.Substring(4));
                    }
                    else {
                        SpawnVehicle(currentlySelectedVehicle.Value.model, true, true, false, currentlySelectedVehicle.Value,
                                     currentlySelectedVehicle.Key.Substring(4));
                    }
                }
                else if (item == renameVehicle) {
                    string newName = await GetUserInput("Enter a new name for this vehicle.", 30);
                    if (string.IsNullOrEmpty(newName)) {
                        ...
                        if (StorageManager.SaveVehicleInfo("veh_" + newName, currentlySelectedVehicle.Value, false)) {
                            ...
                            Notify.Error(
                                "This name is already in use or something unknown failed. Contact the server owner if you believe something is wrong.");
                        }
                    }
                }
            ...
            unavailableVehiclesMenu.ButtonPressHandlers.Add(new Menu.ButtonPressHandler(Control.FrontendDelete, Menu.ControlPressCheckType.JUST_RELEASED,
                                                                                        (m, c) => {
                                                                                            if (m.Size > 0) {
                                                                                                int index = m.CurrentIndex;
                                                                                                if (index < m.Size) {
                                                                                                    MenuItem item = m.GetMenuItems()
                                                                                                        .Find(i => i.Index == index);
                                                                                                    if (item != null &&
                                                                                                        item.ItemData is KeyValuePair<string, VehicleInfo> sd) {
                                                                                                        if (item.Label == "~r~Are you sure?") {
                                                                                                            Log("Unavailable saved vehicle deleted, data: " +
                                                                                                        ...
                                                                                                        Notify.Error(
                                                                                                            "Somehow this vehicle could not be found.");
                                                                                                    }
                                                                                                }
                                                                                                else {
                                                                                                    Notify.Error(
                                                                                                        "You somehow managed to trigger deletion of a menu item that doesn't exist, how...?");
                                                                                                }
                                                                                            }
                                                                                            else {
                                                                                                Notify.Error(
                                                                                                    "There are currrently no unavailable vehicles to delete!");
                                                                                            }
                                                                                        }, true));

            void ResetAreYouSure()
            {
                foreach (MenuItem i in unavailableVehiclesMenu.GetMenuItems()) {
        ...
        bool UpdateSelectedVehicleMenu(MenuItem selectedItem, Menu parentMenu = null)
        {
            if (!svMenuItems.ContainsKey(selectedItem)) {
                Notify.Error(
                    "In some very strange way, you've managed to select a button, that does not exist according to this list. So your vehicle could not be loaded. :( Maybe your save files are broken?");
                return false;
            }
        ...
        public void UpdateMenuAvailableCategories()
        {
            ...
            for (int i = 1; i < GetMenu().Size - 1; i++) {
                if (savedVehicles.Any(a => GetVehicleClassFromName(a.Value.model) == i - 1 && IsModelInCdimage(a.Value.model))) {
                    ...
                    GetMenu().GetMenuItems()[i].Description =
                        $"You do not have any saved vehicles that belong to the {GetMenu().GetMenuItems()[i].Text} category.";
                }
            }
            ...
            foreach (KeyValuePair<string, VehicleInfo> sv in savedVehicles) {
                if (IsModelInCdimage(sv.Value.model)) {
                    ...
                    MenuItem missingVehItem = new MenuItem(sv.Key.Substring(4),
            ...
            foreach (Menu m in subMenus) {
                m.SortMenuItems((A, B) => { return A.Text.ToLower().CompareTo(B.Text.ToLower()); });
            }
        }
