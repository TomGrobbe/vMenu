...
**That license comes along with your plugin.** A plugin built on this package, or on either of the two packages that bring it in, is a work based on vMenu, so your plugin has to be licensed `GPL-3.0` or `AGPL-3.0`, or the "or later" version of either. You cannot build on this and then put your own closed license on the result. In practice:
