...
// Two ways into the same wardrobe, both built by PedCustomizationRows from one row builder. The
// collections share one child menu rather than having one each: there can be dozens of them, they
// are runtime data, and MenuAPI cannot take a menu back out once it has been added.
[VMenu(
...
public sealed class PlayerAppearanceMenu : MenuDefinition
{
    ...
    // The walking styles have to be here before the rows are built, because this menu owns child menus
    // and MenuAPI cannot take one of those back out, so it cannot be rebuilt later.
    ...
    protected override void Build(MenuBuilder menu)
    {
        ...
        // Locked rather than hidden while the ped has only the base game's clothes, which is nearly every
        // ped that is not one of the online character models. Grouping one collection by the update it came
        // from just repeats the page above, and a row that quietly disappears reads as imagined.
        menu.Entries.Add(new SubmenuEntry
        {
        ...
        menu.Entries.Add(new ListEntry
        {
            ...
            // On scroll, unlike the walking style, because nothing has to load and seeing it change as you go is
            // the whole point of a choice about how something looks.
        ...
        menu.OnOpened = _ =>
        {
            // The hint is worth saying once per visit, not once per session, so a player who forgot it half an
            // hour ago is told again the next time they put a helmet on.
            PedVisorHint.Reset();

            // Whether the collections row is open depends on the ped, and the player changes that in another
            // menu entirely. Gates are only re-evaluated on a refresh, and nothing else would ask for one.
            MenuRegistry.RefreshAll();
        };
    }

    // Read on open rather than once, because which collections exist depends entirely on which ped the
    // player is wearing, and they can change that from another menu between visits.
    ...
    // rather than on scroll: each one has to load its animations first, so stepping past five of them
    // would ask for five loads.
    private static ListEntry WalkingStyleRow()
    {
        ...
        // The first choice is always the ped's own walk, so a player can always get back to it even when the
        // owner offers nothing else.
    ...
    private static async Task ApplyWalkAsync(List<MenuText> options, List<string> clipsets, int index)
    {
        ...
        // Nothing loaded under that name for this ped, which is a real thing that happens: a walk animated
        // for a person has nothing to give a coyote.
    ...
    // A collection is the content pack a piece of clothing came from, not a preset outfit. Nearly every
    // ped only has the base game's, because DLC clothing was almost all made for the online character
    // models, and for those peds this page would just repeat Ped Customization.
    private static bool HasSeveralCollections() => PedCollections.All(Native.PlayerPedId()).Count > 1;

    // The game holds, per ped, which combinations of pieces belong together, and this is the only way to
    // reach that. Picking twelve slots at random ourselves would put a police hat on a hiker.
    private static void Randomise()
    {
        var ped = Native.PlayerPedId();

        // The second argument asks the game to keep the combination coherent rather than picking each slot
        // on its own.
