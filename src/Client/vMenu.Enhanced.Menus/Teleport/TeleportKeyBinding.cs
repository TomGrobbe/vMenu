...
/// <summary>
/// One key, doing whichever teleport the player chose for it in the teleport menu.
/// </summary>
// Registered once and never unregistered, unlike the chat commands, because a FiveM key mapping has
// to exist for the player to be able to rebind it under Settings. Taking it away when a permission
// is missing would take the binding out of their settings with it, so the key always exists and
// answers for itself when pressed.
public static class TeleportKeyBinding
{
    /// <summary>The key does nothing. What a player who has never chosen has.</summary>
    ...
    /// <summary>A teleport fades the screen and moves the player, so only one runs at a time.</summary>
    private static bool _running;

    /// <summary>Call after <see cref="ClientConfig.Initialize"/>, whose convar names the key.</summary>
    ...
    /// answers as though nothing is there: <c>IsWaypointActive</c> says there is no waypoint even when
    /// the player is looking at theirs. Everything below touches the game, so none of it runs here.
    /// </summary>
    ...
    private static async void Dispatch()
    {
        ...
        // Checked here rather than by refusing to register the key, so the player keeps their choice
        // and their binding when they move between servers that allow different things.
