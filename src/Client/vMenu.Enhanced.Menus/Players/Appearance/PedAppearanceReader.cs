...
public static class PedAppearanceReader
{
    ...
    private static List<PedComponentValue> ReadComponents(int ped)
    {
        ...
        {
            // A model that has nothing for this slot, such as most animals. Recording a value the model does not
            // have would make every restore report a difference it cannot fix.
            ...
            components.Add(new PedComponentValue
            {
                ...
                // The game answers -1 for a slot whose texture it has not decided on. Stored as zero, because that
                // is the texture it will draw, and -1 is not a value anything can set.
    ...
    private static List<PedPropValue> ReadProps(int ped)
    {
        ...
        {
            // The false is the dead check the enhanced natives added. Off, because a ped is wearing what it is
            // wearing whether or not it happens to be dead.
            var drawable = Native.GetPedPropIndex(ped, slot, false);

            // Nothing worn. Left out of the list entirely rather than stored as legacy's -1, so an empty slot
