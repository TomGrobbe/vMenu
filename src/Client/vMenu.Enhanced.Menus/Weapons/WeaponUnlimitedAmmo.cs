...
public static class WeaponUnlimitedAmmo
{
    ...
    /// <summary>Every weapon the tick has switched this on for, so it can be switched back off again.</summary>
    // The game keeps the flag on the weapon rather than on the ped, so stopping the tick stops it
    // being set again but leaves every weapon it already ran on firing forever.
    ...
    private static void Apply()
    {
        ...
        // Told which weapon rather than left to the flag alone, because the game applies it to the
        // one being held and the player can switch at any moment.
