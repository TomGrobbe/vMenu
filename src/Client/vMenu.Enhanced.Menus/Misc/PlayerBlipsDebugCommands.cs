...
// The fakes are pushed in through the very same handler the server's snapshot arrives at, so nothing
// downstream can tell them apart from a genuine player: a test that took a shortcut past the parsing
// and the staleness rules would not be testing the thing that ships. They are all players the game
// has not streamed in, which is the harder case and the one nobody can produce on their own.
public static class PlayerBlipsDebugCommands
{
    private const string Command = "vmenu_blips_test";

    // Far above anything FiveM will ever hand out, so a fake can never be mistaken for a real player or
    // quietly overwrite one in the store.
    private const int FirstFakeId = 900_001;

    private const int MaxFakes = 64;

    // A stored position is only trusted for fifteen seconds, which is what stops a dropped message
    // leaving a blip on the map forever, and a fake nobody keeps repeating is subject to it too.
    ...
    // dimming past that, and off the ordinary minimap past 1000m.
    private const float NearestMetres = 120f;

    private const float FurthestMetres = 3000f;

    // Roughly 137.5 degrees, the angle that spreads points around a centre most evenly. Any whole
    // fraction of a circle lines the fakes up into spokes, hiding one ring of distances behind another.
    private const float GoldenAngle = 2.399963f;

    // Empty means on foot. The ordinary car and the pushbike are in here on purpose: both should stay a
    // plain dot, and a test that only contains special cases never catches the day it stops.
    ...
    private static void Spawn(int count)
    {
        ...
        {
            // Spread evenly over the whole range rather than placed at random, so the same command twice puts a
            // blip at the same distance twice and you can compare the two.
            ...
            Fakes.Add(new Fake(
                ...
                // Every third one, so the staff ring and the plain blip sit next to each other on the map.
                index % 3 == 0,
    ...
    private sealed class Fake(int serverId, Vector3 position, uint model, bool staff, string name)
    {
        ...
        // Degrees per push, varied per fake so they do not all turn in lockstep.
