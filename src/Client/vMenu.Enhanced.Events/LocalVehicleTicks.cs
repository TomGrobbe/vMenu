...
/// <summary>What the player is sitting in, and what state it is in.</summary>
public static class LocalVehicleTicks
{
    /// <summary>The seat the game calls the driver's. Passengers are 0 and up.</summary>
    private const int DriverSeat = -1;

    /// <summary>On foot, or in a vehicle without answering to any of its seats.</summary>
    // Below the driver's index, so it can never collide with a real seat.
    private const int NoSeat = -2;

    /// <summary>
    /// Getting out of a car takes about a second and a half of animation, so a quarter second after
    /// that lands well under what a player notices as the menu catching up.
    /// </summary>
    ...
    /// <summary>As <see cref="VehicleEntered"/>, for a handler that needs to await something.</summary>
    ...
    /// <summary>The player is on foot again, whether they climbed out or the vehicle went away.</summary>
    ...
    /// <summary>As <see cref="VehicleExited"/>, for a handler that needs to await something.</summary>
    ...
    /// <summary>The player went straight from one vehicle into another.</summary>
    ...
    /// <summary>As <see cref="VehicleSwapped"/>, for a handler that needs to await something.</summary>
    ...
    /// <summary>Same vehicle, different seat.</summary>
    ...
    /// <summary>As <see cref="VehicleSeatChanged"/>, for a handler that needs to await something.</summary>
    ...
    /// <summary>Any of the four above, for a listener that only cares that the answer moved.</summary>
    ...
    /// <summary>As <see cref="VehicleChanged"/>, for a handler that needs to await something.</summary>
    ...
    /// <summary>The vehicle the player is in lost body, engine or petrol tank health.</summary>
    ...
    /// <summary>As <see cref="VehicleDamaged"/>, for a handler that needs to await something.</summary>
    ...
    private static void PollSeat()
    {
        ...
        // Written before the dispatch, never after: a subscriber that throws, or one that calls back
        // in here, must not be able to make the same change fire twice.
        _vehicle = vehicle;
        _seat = seat;
    ...
    /// <summary>What the player is in and which seat of it, with 0 and <see cref="NoSeat"/> for on foot.</summary>
    private static void Read(out int vehicle, out int seat)
    {
        ...
        // The false is "not while climbing in". GetVehiclePedIsIn on its own answers with the vehicle
        // a ped is still walking towards, and a section that refilled then would refill again on
        // arrival.
        ...
        // A vehicle deleted or streamed out from under the player leaves a handle nothing answers to,
        // and a menu needs to hear about that as leaving it.
    ...
    /// <summary>Which seat of a vehicle a ped is in, or <see cref="NoSeat"/> if none of them answer.</summary>
    private static int FindSeat(int vehicle, int ped)
    {
        // The same question VehicleTargeting asks, so the watcher and the menus cannot end up
        // disagreeing about who is driving.
    ...
    // Three separate numbers with three different ranges: body and petrol tank run 0 to 1000, and
    // engine goes down to about minus four thousand once it is dead or on fire.
