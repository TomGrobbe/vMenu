...
internal static class GitHubSource
{
    ...
    // The release list, which is the only endpoint that can see an enhanced release. /releases/latest is
    // useless here: it excludes prereleases and drafts, and every enhanced release so far is at least one
    // of those. per_page keeps the body down, the release notes being most of it.
    ...
    // Unauthenticated GitHub allows 60 requests an hour from one address, and this makes four a day on
    // the schedule. A shared host that does hit the limit gets a 403, which is a debug line and a fall
    // through to nuget.org rather than a warning.
    public static async Task<SourceResult> LatestAsync(UpdateChannel channel, string userAgent, int timeoutMs)
    {
        var reply = await HttpSend.SendAsync(new HttpRequest(ReleasesUrl, Accept, userAgent, timeoutMs));
        ...
        {
            // Anonymous callers never see drafts anyway. Checked because a server that ever runs this with a
            // token in the environment would start seeing them.
            ...
            // Belt and braces over the release flag: the combined workflow does not pass --prerelease, so a
            // combined release carrying an alpha enhanced build reads as stable at the GitHub level while its
            // version says otherwise. The version wins.
    ...
    // StartsWith and not Contains: combined-v3.9.0-and-enhanced-v0.0.2 contains enhanced-v in the middle,
    // so a Contains check would take the wrong substring. Legacy v3.x.y tags fall through to null.
