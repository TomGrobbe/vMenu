...
/// <summary>
/// One GET over <see cref="HttpClient" />, started on the thread pool and waited on the tick thread
/// so everything the caller does after it can still call natives.
/// </summary>
public static class HttpGet
{
    // One client for the resource's lifetime. Built on first use rather than at type load, so a
    // constructor that ever throws in this runtime turns into an unusable reply instead of a class
    // that will not load.
    ...
    public static async Task<HttpReply> GetAsync(HttpRequest request)
    {
        ...
        // Fire and forget: the request runs on the thread pool and drops its answer into the slot,
        // while the caller waits for it on the tick thread through HttpWait. That is deliberate,
        // because the caller calls natives the moment it has the reply, and only the tick thread may.
    ...
    {
        // DateTimeOffset and not GetGameTimer: everything past the first await runs off the tick
        // thread, where a native call is not allowed, and the clock is fine to read from anywhere.
    ...
    // Microsoft.Win32.Registry, and that assembly is not shipped next to the resource so the load
    // throws "could not load file or assembly". vMenu talks straight to github.com and nuget.org,
    // so there is nothing a proxy would do here anyway.
