...
// How long a tick waits between iterations. The wait happens after the handler returns rather than
// on a timer, so a slow iteration delays the next instead of overlapping with it. That is the whole
// difference from API.SetInterval.
public readonly struct TickRate
{
    ...
    // Once per frame on the client, once per server tick on the server.
    public static TickRate PerFrame => default;

    public static TickRate Every(long milliseconds) => new(milliseconds);

    // Asked again after every iteration, for a loop that only sometimes needs the fast rate.
