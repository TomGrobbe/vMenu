...
// Where the moon sits in its cycle, which follows from the in-game date and nothing else. GTA offsets
// the moon's angle by PI * ((fmod(cycleTime, 55) - 27) / 27), where cycleTime is whole days since
// 1 January 2000 plus the fraction of the current day. Everything here is that one formula in
// friendlier units, so the angle is the number to trust and the rest is read off it.
public static class MoonCycle
{
    // In-game days from one new moon to the next.
    public const double CycleDays = 55.0;

    // The day the angle passes through zero, which is the moon fully lit.
    public const double FullMoonDay = 27.0;

    // The moon's 55 day cycle and the 7 day week, so the date can wrap without either jumping. Keeping
    // the day count a multiple of 55 and 7 apart from the true one leaves both cycles where they were.
    public const long PeriodDays = 385;

    // 1 January 2000 in days since the Unix epoch, the date the cycle measures from.
    ...
    // 1 January 1970 was a Thursday, which is where a week counted in Unix days starts.
