...
namespace vMenuServer {
        ...
        public MainServer()
        {
            ...
            if (GetCurrentResourceName() != "vMenu") {
                Exception InvalidNameException = new Exception("\r\n\r\n^1[vMenu] INSTALLATION ERROR!\r\nThe name of the resource is not valid. " +
                                                               "Please change the folder name from '^3" + GetCurrentResourceName() +
                                                               "^1' to '^2vMenu^1' (case sensitive) instead!\r\n\r\n\r\n^7");
                try {
    ...
    #region command handler
        ...
        void ServerCommandHandler(int source, List<object> args, string rawCommand)
        {
            if (args != null) {
                if (args.Count > 0) {
                    switch (args[0].ToString().ToLower()) {
                        ...
                        case "unban" when source < 1: {
                            if (args.Count() > 1 && !string.IsNullOrEmpty(args[1].ToString())) {
                                string uuid = args[1].ToString().Trim();
                                List<BanManager.BanRecord> bans = BanManager.GetBanList();
                                BanManager.BanRecord banRecord = bans.Find(b => { return b.uuid.ToString() == uuid; });
                                if (banRecord != null) {
                                ...
                                Debug.WriteLine(
                                    "You did not specify a player to unban, you must enter the FULL playername. Usage: vmenuserver unban \"playername\"");
                            }

                            break;
                        }
                        case "weather" when args.Count < 2 || string.IsNullOrEmpty(args[1].ToString()):
                                ...
                                if (int.TryParse(args[2].ToString(), out int minute)) {
                                    if (hour >= 0 && hour < 24) {
                                        if (minute >= 0 && minute < 60) {
                                            ...
                                            Debug.WriteLine(
                                                $"Time is now {(hour < 10 ? "0" + hour : hour.ToString())}:{(minute < 10 ? "0" + minute : minute.ToString())}.");
                                        }
                                        else {
                        ...
                        case "time":
                            Debug.WriteLine("Invalid syntax. Use: ^5vmenuserver time <freeze|<hour> <minute>>^7 instead.");
                            break;
                        // only do this via server console (server id < 1)
                        case "ban" when source < 1: {
                            if (args.Count > 3) {
                                Player p = null;
                                ...
                                if (p != null) {
                                    ...
                                    p.Drop(
                                        $"You are banned from this server. Ban time remaining: {timeRemaining}. Banned by: {ban.bannedBy}. Ban reason: {ban.banReason}. Additional information: {GetSettingsString(Setting.vmenu_default_ban_message_information)}.");
                                }
                                else {
                        ...
                        case "help":
                            ...
                            Debug.WriteLine(
                                "vmenuserver migrate (This copies all banned players in the bans.json file to the new ban system in vMenu v3.3.0, you only need to do this once)");
                            break;
                        case "migrate" when source < 1: {
                            ...
                            Debug.WriteLine(
                                "^5[vMenu] [INFO]^7 Importing all ban records from the bans.json file into the new storage system. ^3This may take some time...^7");
                            List<BanManager.BanRecord> bans = JsonConvert.DeserializeObject<List<BanManager.BanRecord>>(file);
                            bans.ForEach(br => {
                                BanManager.BanRecord record =
                                    new BanManager.BanRecord(br.playerName, br.identifiers, br.bannedUntil, br.banReason, br.bannedBy, Guid.NewGuid());
    ...
    #region kick players from personal vehicle
        ...
        void GetOutOfCar([FromSource] Player source, int vehicleNetId, int playerOwner)
        {
            if (source != null) {
                if (PermissionsManager.GetPermissionAndParentPermissions(PermissionsManager.Permission.PVKickPassengers)
                                      .Any(perm => PermissionsManager.IsAllowed(perm, source))) {
                    TriggerClientEvent("vMenu:GetOutOfCar", vehicleNetId, playerOwner);
                    source.TriggerEvent(
                        "vMenu:Notify",
                        "All passengers will be kicked out as soon as the vehicle stops moving, or after 10 seconds if they refuse to stop the vehicle.");
                }
            }
        }
    #endregion
    ...
    #region clear area near pos
        ...
        void ClearAreaNearPos(float x, float y, float z)
        {
    ...
    #region logging and update checks notifications
        ...
        static void KickLog(string kickLogMesage)
        {
    ...
    #region Add teleport location
        ...
        void AddTeleportLocation([FromSource] Player source, string locationJson)
        {
    ...
    #region vars
        ...
        public static string Version => GetResourceMetadata(GetCurrentResourceName(), "version", 0);

        // Time
        int CurrentHours {
            get => MathUtil.Clamp(GetSettingsInt(Setting.vmenu_current_hour), 0, 23);
            set => SetConvarReplicated(Setting.vmenu_current_hour.ToString(), MathUtil.Clamp(value, 0, 23).ToString());
        }

        int CurrentMinutes {
            get => MathUtil.Clamp(GetSettingsInt(Setting.vmenu_current_minute), 0, 59);
            set => SetConvarReplicated(Setting.vmenu_current_minute.ToString(), MathUtil.Clamp(value, 0, 59).ToString());
        }

        int MinuteClockSpeed {
            get {
                int value = GetSettingsInt(Setting.vmenu_ingame_minute_duration);
                if (value < 100) {
                    value = 2000;
                }
        ...
        bool FreezeTime {
            get => GetSettingsBool(Setting.vmenu_freeze_time);
            set => SetConvarReplicated(Setting.vmenu_freeze_time.ToString(), value.ToString().ToLower());
        }

        bool IsServerTimeSynced => GetSettingsBool(Setting.vmenu_sync_to_machine_time);


        // Weather
        string CurrentWeather {
            get {
                string value = GetSettingsString(Setting.vmenu_current_weather, "CLEAR");
        ...
        bool DynamicWeatherEnabled {
            get => GetSettingsBool(Setting.vmenu_enable_dynamic_weather);
            set => SetConvarReplicated(Setting.vmenu_enable_dynamic_weather.ToString(), value.ToString().ToLower());
        }

        bool BlackoutEnabled {
            get => GetSettingsBool(Setting.vmenu_blackout_enabled);
            set => SetConvarReplicated(Setting.vmenu_blackout_enabled.ToString(), value.ToString().ToLower());
        }

        int DynamicWeatherMinutes => Math.Max(GetSettingsInt(Setting.vmenu_dynamic_weather_timer), 1);

        long lastWeatherChange;

        readonly List<string> CloudTypes = new List<string> {
            "Cloudy 01",
            "RAIN",
            "horizonband1",
            "horizonband2",
        ...
        readonly List<string> WeatherTypes = new List<string> {
            "EXTRASUNNY",
            "CLEAR",
            "NEUTRAL",
            "SMOG",
    ...
    #region Sync weather & time with clients
        ...
        void UpdateWeather(string newWeather, bool blackoutNew, bool dynamicWeatherNew)
        {
        ...
        void UpdateWeatherCloudsType(bool removeClouds)
        {
            if (removeClouds) {
        ...
        void UpdateTime(int newHours, int newMinutes, bool freezeTimeNew)
        {
    ...
    #region Online Players Menu Actions
        ...
        void KickPlayer([FromSource] Player source, int target, string kickReason = "You have been kicked from the server.")
        {
        ...
        void KillPlayer([FromSource] Player source, int target)
        {
        ...
        void SummonPlayer([FromSource] Player source, int target)
        {
        ...
        void SendPrivateMessage([FromSource] Player source, int targetServerId, string message)
        {
            ...
            if (targetPlayer != null) {
                ...
                foreach (Player p in Players) {
                    if (p != source && p != targetPlayer) {
                        if (PermissionsManager.IsAllowed(PermissionsManager.Permission.OPSeePrivateMessages, p)) {
        ...
        void NotifySenderThatDmsAreDisabled([FromSource] Player source, string senderServerId)
        {
            Player p = Players[int.Parse(senderServerId)];
            if (p != null) {
                p.TriggerEvent("vMenu:Notify",
                               $"Sorry, your private message to <C>{source.Name}</C>~s~ could not be delivered because they disabled private messages.");
            }
        }
    #endregion

    #region Infinity bits
        void RequestServerStateFromPlayer([FromSource] Player player)
        {
        ...
        void RequestPlayerListFromPlayer([FromSource] Player player)
        {
        ...
        void GetPlayerCoords([FromSource] Player source, int playerId, NetworkCallbackDelegate callback)
        {
            if (IsPlayerAceAllowed(source.Handle, "vMenu.OnlinePlayers.Teleport") || IsPlayerAceAllowed(source.Handle, "vMenu.Everything") ||
                IsPlayerAceAllowed(source.Handle, "vMenu.OnlinePlayers.All")) {
                Vector3 coords = Players[playerId]?.Character?.Position ?? Vector3.Zero;
    ...
    #region Player join/quit
        HashSet<string> joinedPlayers = new HashSet<string>();

        Task PlayersFirstTick()
        {
            Tick -= PlayersFirstTick;
        ...
        void OnPlayerJoining([FromSource] Player sourcePlayer)
        {
        ...
        void OnPlayerDropped([FromSource] Player sourcePlayer, string reason)
        {
